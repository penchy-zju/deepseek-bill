...
- **某一天的数据在当天结束后仍可能被平台修订**：实测同一天相隔几小时的两次导出，Token 量、请求数、费用与逐小时分布**完全一致**，但个别行归属的 API Key 变了（同一条用量从一个 Key 挪到另一个 Key）。这意味着历史某天的文件会偶发地被更新一次（提交信息仍是 `data: amount YYYY-MM-DD`）。如果你要拿这些 CSV 做对账，建议以较晚的版本为准
...
| 情况 | 行为 |
| --- | --- |
| 该天没有任何调用（接口只返回表头） | 记一条 `::warning::`，**不写文件、不提交**，其余步骤正常继续 |
| 接口返回的 `amount.csv` 是 0 字节 | 同上（没有表头也没有数据行，视为“无数据”而非报错） |
| 压缩包里**找不到** amount CSV（只有 cost / 空压缩包） | **报错退出**（这是接口变动，不能当成“无数据”） |
| 整个仓库一天数据都没有 | 看板渲染空状态（“data/ 目录下还没有 amount-*.csv 数据文件”），校验步骤仍然通过 |
| 有真实数据 + 某天只有表头 | 正常按已有数据出图，只有表头的那天不出现在日期列表里 |

> 已知取舍：因为「没有用量的那天」不会留下任何文件，看板上也就**没有这一天**。所以从看板无法区分「那天真的没有用量」和「那天的抓取失败了」。如需区分，可让空天也留下一个只有表头的文件、并在页面上把它显示为「无数据」—— 目前保持现状。
