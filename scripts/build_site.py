...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  /* 所有单元格统一：数字列右对齐（数字等宽，右缘必然对齐），第一列文字左对齐 */
  ...
  /* 对齐规则（统一口径，避免同一列里有的左、有的右）：
     1) 默认所有单元格右对齐 —— 数字等宽，右缘自然对齐；
     2) 第 1 列是文字（模型 / API Key / 小计 / 合计）→ 左对齐；
     3) 「全部 API Key 对比」的模型列也是文字 → 用 .model-cell 标记后左对齐。
     注意：第 2、3 条必须用语义选择器，不能用 :nth-child()：
     跨行（rowspan）单元格的续行里，第 2 个 DOM 子元素其实是「调用次数」数字格，
     按位置选会把该列数字变成左对齐，与同列其它行不一致。 */
  th:nth-child(1), td:nth-child(1) { text-align:left; }
  #compareTable th.model-cell, #compareTable td.model-cell { text-align:left; }
  tr.total td.subtotal-label { text-align:left; color:var(--ink); }
  tbody tr:hover { background:var(--accent-soft); }
  /* 合计/小计行：加深色顶边框 + 浅色底，形成明显分隔 */
  tr.total td { font-weight:650; background:var(--accent-soft); border-bottom:1px solid var(--line); }
  /* 小计行是「一组模型的收尾」，用更重的线把它和下一个 Key 隔开 */
  #compareTable tr.total td { border-top:2px solid var(--muted); }
  #compareTable tr.grand td { border-top:2px solid var(--ink); }
  /* 序号/名称列：与小计行的左边界对齐，形成视觉上的分组 */
  #compareTable td:nth-child(1) { border-left:3px solid var(--accent); }
  /* 同一个 Key 的多行：左侧加一条竖线，形成明显的分组归属 */
  #compareTable tbody tr td.key-cell { border-left:3px solid var(--line); box-shadow:-4px 0 6px -5px rgba(0,0,0,.28); }
